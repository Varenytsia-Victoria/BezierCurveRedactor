...
function calculateBezierPoint(t, controlPoints) {
    ...
    for (let i = 0; i <= n; i++) {
        result += binomialCoefficient(n, i) * Math.pow((1 - t), n - i) * Math.pow(t, i) * controlPoints[i]; //обчислює значення кривої в певній точці
    }
...
function drawCharacteristicLine(lineColor) {
    ...
    if (points.length > 0) {
        ...
        // Малювання точок для контрольних точок
        ...
        if (validPointsX.length > 0 && validPointsY.length > 0) {
            // малювання лінії з першої дійсної точки
            context.moveTo(validPointsX[0], validPointsY[0]);

            // Малювання лінії між дійсними точками
...
function drawMatrixBezierCurve(controlPointsX, controlPointsY) {
    ...
    context.strokeStyle = characteristicLinePathColor;  
    context.stroke();
    context.closePath();
